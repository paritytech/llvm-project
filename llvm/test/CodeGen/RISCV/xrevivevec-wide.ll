; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -mtriple=riscv64 -mattr=+m,+xrevivevec -verify-machineinstrs < %s \
; RUN:   | FileCheck %s

; i512 and i1024 are machine types too, in VRM4 and VRM8. The instruction set is
; the same at every width; only the opcode space and register class change.

define i512 @add_512(i512 %a, i512 %b) {
; CHECK-LABEL: add_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dadd v8, v8, v12
; CHECK-NEXT:    ret
  %r = add i512 %a, %b
  ret i512 %r
}

define i1024 @add_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: add_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qadd v8, v8, v16
; CHECK-NEXT:    ret
  %r = add i1024 %a, %b
  ret i1024 %r
}

define i512 @mul_512(i512 %a, i512 %b) {
; CHECK-LABEL: mul_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dmul v8, v8, v12
; CHECK-NEXT:    ret
  %r = mul i512 %a, %b
  ret i512 %r
}

define i1024 @mul_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: mul_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qmul v8, v8, v16
; CHECK-NEXT:    ret
  %r = mul i1024 %a, %b
  ret i1024 %r
}

; MaxDivRemBitWidthSupported covers every width, so no libcall.
define i512 @udiv_512(i512 %a, i512 %b) {
; CHECK-LABEL: udiv_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.ddivu v8, v8, v12
; CHECK-NEXT:    ret
  %r = udiv i512 %a, %b
  ret i512 %r
}

define i1024 @sdiv_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: sdiv_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qdiv v8, v8, v16
; CHECK-NEXT:    ret
  %r = sdiv i1024 %a, %b
  ret i1024 %r
}

define i512 @srem_512(i512 %a, i512 %b) {
; CHECK-LABEL: srem_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.drem v8, v8, v12
; CHECK-NEXT:    ret
  %r = srem i512 %a, %b
  ret i512 %r
}

define i1024 @urem_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: urem_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qremu v8, v8, v16
; CHECK-NEXT:    ret
  %r = urem i1024 %a, %b
  ret i1024 %r
}

define i512 @shl_512(i512 %a, i64 %b) {
; CHECK-LABEL: shl_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dsll v8, v8, a0
; CHECK-NEXT:    ret
  %s = zext i64 %b to i512
  %r = shl i512 %a, %s
  ret i512 %r
}

define i1024 @ashr_1024(i1024 %a, i64 %b) {
; CHECK-LABEL: ashr_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qsra v8, v8, a0
; CHECK-NEXT:    ret
  %s = zext i64 %b to i1024
  %r = ashr i1024 %a, %s
  ret i1024 %r
}

define i64 @cmp_ult_512(i512 %a, i512 %b) {
; CHECK-LABEL: cmp_ult_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dsltu a0, v8, v12
; CHECK-NEXT:    ret
  %c = icmp ult i512 %a, %b
  %r = zext i1 %c to i64
  ret i64 %r
}

define i64 @cmp_eq_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: cmp_eq_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qseq a0, v8, v16
; CHECK-NEXT:    ret
  %c = icmp eq i1024 %a, %b
  %r = zext i1 %c to i64
  ret i64 %r
}

; The select pseudo is per register class, so each width needs its own.
define i512 @select_512(i1 %c, i512 %a, i512 %b) {
; CHECK-LABEL: select_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    andi a0, a0, 1
; CHECK-NEXT:    bnez a0, .LBB12_2
; CHECK-NEXT:  # %bb.1:
; CHECK-NEXT:    vsetivli zero, 1, e8, m1, ta, ma
; CHECK-NEXT:    vmv4r.v v8, v12
; CHECK-NEXT:  .LBB12_2:
; CHECK-NEXT:    ret
  %r = select i1 %c, i512 %a, i512 %b
  ret i512 %r
}

define i1024 @select_1024(i1 %c, i1024 %a, i1024 %b) {
; CHECK-LABEL: select_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    andi a0, a0, 1
; CHECK-NEXT:    bnez a0, .LBB13_2
; CHECK-NEXT:  # %bb.1:
; CHECK-NEXT:    vsetivli zero, 1, e8, m1, ta, ma
; CHECK-NEXT:    vmv8r.v v8, v16
; CHECK-NEXT:  .LBB13_2:
; CHECK-NEXT:    ret
  %r = select i1 %c, i1024 %a, i1024 %b
  ret i1024 %r
}

define i512 @load_512(ptr %p) {
; CHECK-LABEL: load_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dld v8, 0(a0)
; CHECK-NEXT:    ret
  %r = load i512, ptr %p
  ret i512 %r
}

define void @store_1024(ptr %p, i1024 %v) {
; CHECK-LABEL: store_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qst v8, 0(a0)
; CHECK-NEXT:    ret
  store i1024 %v, ptr %p
  ret void
}

define i512 @zext_512(i64 %a) {
; CHECK-LABEL: zext_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dzext v8, a0
; CHECK-NEXT:    ret
  %r = zext i64 %a to i512
  ret i512 %r
}

define i1024 @sext_1024(i64 %a) {
; CHECK-LABEL: sext_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qsext v8, a0
; CHECK-NEXT:    ret
  %r = sext i64 %a to i1024
  ret i1024 %r
}

define i64 @trunc_1024(i1024 %a) {
; CHECK-LABEL: trunc_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qtrunc a0, v8
; CHECK-NEXT:    ret
  %r = trunc i1024 %a to i64
  ret i64 %r
}

; Cross-width conversions are subregister operations.
define i512 @zext_256_to_512(i256 %a) {
; CHECK-LABEL: zext_256_to_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dzext v12, zero
; CHECK-NEXT:    vsetivli zero, 1, e8, m1, ta, ma
; CHECK-NEXT:    vmv2r.v v12, v8
; CHECK-NEXT:    vmv4r.v v8, v12
; CHECK-NEXT:    ret
  %r = zext i256 %a to i512
  ret i512 %r
}

define i256 @trunc_1024_to_256(i1024 %a) {
; CHECK-LABEL: trunc_1024_to_256:
; CHECK:       # %bb.0:
; CHECK-NEXT:    ret
  %r = trunc i1024 %a to i256
  ret i256 %r
}

define i512 @umax_512(i512 %a, i512 %b) {
; CHECK-LABEL: umax_512:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.dmaxu v8, v8, v12
; CHECK-NEXT:    ret
  %c = icmp ugt i512 %a, %b
  %r = select i1 %c, i512 %a, i512 %b
  ret i512 %r
}

define i1024 @smin_1024(i1024 %a, i1024 %b) {
; CHECK-LABEL: smin_1024:
; CHECK:       # %bb.0:
; CHECK-NEXT:    revive.qmin v8, v8, v16
; CHECK-NEXT:    ret
  %c = icmp slt i1024 %a, %b
  %r = select i1 %c, i1024 %a, i1024 %b
  ret i1024 %r
}
